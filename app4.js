...
function updateDateTime() {
  ...
  // Menggunakan new Date() dan menampilkan dengan toLocaleString yang benar untuk zona waktu.
  const now = new Date(); 
  const localDate = now.toLocaleString("en-US", { timeZone: "Asia/Jakarta" });
  const jakartaTime = new Date(localDate); // Membuat objek Date baru di zona waktu Jakarta

  const dayName = days[jakartaTime.getDay()];
  const date = jakartaTime.getDate();
  const month = months[jakartaTime.getMonth()];
  const year = jakartaTime.getFullYear();
  const hh = jakartaTime.getHours().toString().padStart(2, "0");
  const mm = jakartaTime.getMinutes().toString().padStart(2, "0");
  const ss = jakartaTime.getSeconds().toString().padStart(2, "0");
...
// Tambahkan 'profile' ke daftar halaman yang diinisialisasi
["home", "hash", "block", "chain", "ecc", "consensus", "profile"].forEach((p) => {
  const t = document.getElementById("tab-" + p);
  if (t) t.onclick = () => showPage("page-" + p);
});

// ================== HOME PAGE INTERACTION ==================
// FIX: Membuat tombol 'BLOCKCHAIN IS THE FUTURE' menuju Hash Page
document.getElementById('btn-home-to-hash').addEventListener('click', () => {
    showPage('page-hash'); 
});
...
// FIX: Tambahkan blockNumber agar bisa didengarkan perubahannya
...
// FIX: Tambahkan listener untuk blockNumber agar hash berubah saat index diubah
blockNumber.addEventListener("input", updateBlockHash);


async function updateBlockHash() {
  // FIX: Tambahkan blockNumber.value ke data yang di-hash
  const blockIdx = blockNumber.value || "0";
  const data = blockData.value;
  const nonce = blockNonce.value || "0";
  // Asumsi: HASH = SHA256( Index + Data + Nonce )
  blockHash.textContent = await sha256(blockIdx + data + nonce);
}
...
document.getElementById("btn-mine").addEventListener("click", async () => {
  ...
  // Menggunakan toLocaleString untuk mendapatkan waktu Jakarta
  ...
  blockNonce.value = "0";
  let nonce = 0;
  if (status) status.textContent = "INITIATING MINING...";
  async function mineStep() {
    const promises = [];
    for (let i = 0; i < batchSize; i++) {
      // FIX: Tambahkan blockIdx ke data yang di-hash untuk proses mining
      promises.push(sha256(blockIdx + data + timestamp + (nonce + i)));
    }
...
function renderChain() {
  ...
  blocks.forEach((blk, i) => {
    ...
    div.innerHTML = `
      ...
      <button onclick="mineChainBlock(${i})" class="mine" style="width: 100%; margin-top: 10px;">MINE BLOCK</button> 
...
window.mineChainBlock = function (i) {
  ...
  status.textContent = "PROCESSSING BLOCK...";
  async function step() {
    const promises = [];
    ...
    for (let j = 0; j < results.length; j++) {
      ...
      if (h.startsWith(difficulty)) {
        ...
        // Perbarui hash blok berikutnya jika ada
        if (i + 1 < blocks.length) {
            blocks[i + 1].previousHash = blk.hash;
            blocks[i + 1].nonce = 0; // Reset blok berikutnya
            blocks[i + 1].timestamp = ""; // Reset blok berikutnya
            blocks[i + 1].hash = ""; // Reset blok berikutnya
        }
        renderChain(); // Render ulang untuk menampilkan perubahan
        return;
      }
    }
    blk.nonce += batchSize;
  ...
  try {
    ...
    for (let i = 0; i < maxLen; i++) {
      ...
      for (const u of users) {
        ...
        // PERBAIKAN UTAMA: Perbaiki previousHash untuk BLOK BERIKUTNYA
        // Ini harus dilakukan terlepas apakah blok ke-i saat ini awalnya invalid atau tidak.
        // Tujuannya agar rantai selalu merujuk ke blok mayoritas yang sudah dikonfirmasi.
        if (i + 1 < chain.length) {
          chain[i + 1].prev = chain[i].hash;
        }
      }
    }
    ...
    renderConsensusChains();
    alert(
      "Konsensus selesai: blok invalid diganti dengan blok mayoritas yang valid, dan rantai berikutnya diperbaiki."
    );
  } catch (e) {
    console.error(e);
...
const openCV = document.getElementById("btn-open-cv");
const closeCV = document.getElementById("btn-close-cv");
